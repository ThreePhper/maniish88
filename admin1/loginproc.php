...
require_once("../db/connection_mysqli.php"); //connection details
...
if (strlen($password) < 5) { //checking if password is greater then 8 or not
...
if ($status == "OK") {
    ...
    $query = "SELECT * FROM affiliateuser WHERE (user_id = '" . mysqli_real_escape_string($con, $_POST['username']) . "') AND (password = '" . mysqli_real_escape_string($con, $_POST['password']) . "') AND (active = '" . mysqli_real_escape_string($con, "1") . "') AND (level = '" . mysqli_real_escape_string($con, "1") . "')";
